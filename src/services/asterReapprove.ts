...
export async function reapproveAsterForUser(user: {
  ...
  // Cast: `asterAgentEncryptedPK` exists in the production DB column but is
  // not yet reflected in prisma/schema.prisma. The existing
  // /api/aster/approve handler in server.ts uses the same untyped write,
  // so this matches the codebase's de-facto convention. TODO: add the
  // column to the Prisma schema in a follow-up so this can be properly
  // typed across the whole codebase.
