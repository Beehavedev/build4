...
export async function handleWalletCommand(ctx: Context) {
  ...
  // Same address works on every EVM chain — pull BSC, Arbitrum and XLayer
  // balances in parallel. Each is failure-isolated so a single down RPC
  // never blocks the /wallet command from rendering.
