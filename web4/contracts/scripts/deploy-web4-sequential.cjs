...
async function main() {
  ...
  const hub = await Hub.deploy({ nonce: nonce++ });
  ...
  const constitution = await Constitution.deploy({ nonce: nonce++ });
  ...
  const marketplace = await Marketplace.deploy(hubAddress, deployer.address, { nonce: nonce++ });
  ...
  const replication = await Replication.deploy(hubAddress, { nonce: nonce++ });
  ...
  let tx = await hub.authorizeModule(marketplaceAddress, true, { nonce: nonce++ });
  await tx.wait();
  console.log("SkillMarketplace authorized on Hub");
  await sleep(2000);

  tx = await hub.authorizeModule(replicationAddress, true, { nonce: nonce++ });
  await tx.wait();
  console.log("AgentReplication authorized on Hub");
  await sleep(2000);

  tx = await marketplace.setLineageContract(replicationAddress, { nonce: nonce++ });
  await tx.wait();
