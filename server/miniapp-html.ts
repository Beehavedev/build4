...
function renderDeposit(){
  ...
  if(!D.asterApiWallet){
    ...
    h+='<div style="display:flex;gap:8px;align-items:flex-start;margin-bottom:10px"><span style="background:var(--yellow);color:#000;border-radius:50%;min-width:20px;height:20px;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700">6</span><span><strong style="color:var(--yellow)">Important:</strong> Transfer from <strong style="color:#fff">Spot → Futures</strong> on Aster\'s website (Assets → Transfer) or use the button below. Deposits go to Spot by default!</span></div>';
    h+='</div>';
