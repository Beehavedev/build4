...
import {
  ...
  Bot, Sparkles, Briefcase, ArrowRight, ExternalLink,
} from "lucide-react";
import { apiRequest, queryClient } from "@/lib/queryClient";
import type { AgentSkill } from "@shared/schema";
  ...
  return (
    <Card className="bg-black/40 border border-white/10 hover:border-white/25 transition-all duration-300 p-5 flex flex-col gap-3" data-testid={`card-skill-${skill.id}`}>
      <div className="flex items-start justify-between gap-2">
        ...
        {skill.isExecutable && (
          <Badge className="bg-emerald-500/20 text-emerald-400 border-emerald-500/30 text-[10px] px-1.5 shrink-0" data-testid={`badge-executable-${skill.id}`}>
...
function TryItPanel({ skill, onClose }: { skill: EnrichedSkill; onClose: () => void }) {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] text-white">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 py-6">
        ...
        {tryingSkill && (
          <div className="mb-8">
            <TryItPanel skill={tryingSkill} onClose={() => setTryingSkill(null)} />
          </div>
        )}
