...
- **Features**: Individual dashboard, deposit page, quick deposit buttons, manual and AI-assisted trading. AI agent uses Claude Sonnet as core brain for ALL trade decisions (not just a filter). Multi-timeframe analysis (5m/15m/1h) with key level detection. Iron-clad risk: max 1% per trade (code-enforced), on-exchange STOP_MARKET orders, daily loss limit (default 3%), circuit breaker (3 consecutive losses → 30min pause), 5min cooldown after closes (anti-revenge), position sizing based on ATR volatility and stop distance. Claude provides full reasoning for every decision. Market regime detection (TRENDING/RANGING/VOLATILE via ADX+Choppiness Index) blocks RANGING trades. Live reasoning displayed in Mini App with decision log.
- **Adaptive Learning System**: Learns from every trade with 4 mechanisms: (1) Dynamic per-symbol confidence thresholds — raises threshold for underperforming pairs, lowers for strong ones; (2) Time-of-day patterns — blocks historically poor trading hours (<30% win rate), identifies best hours; (3) Regime-aware strategy — tracks win rates per regime and per symbol-regime combo, feeds insights to AI; (4) Correlation blocking — prevents same-direction entries on correlated pairs (BTC↔ETH, SOL↔SUI, DOGE↔XRP, ADA↔AVAX↔LINK). All learning data persisted to DB and displayed in Mini App "Adaptive Learning" panel.
