...
- **AI Agent Badge (GMGN)**: ERC-8004 Identity Registry for AI badge on GMGN. BSC contract (`0x8004A818BFB912233c491871b3d84c89A494BD9e`) is an ERC-1967 proxy with uninitialized implementation — BSC registration detects this via `name()` probe and returns a clear error instead of wasting gas. Registration works on Base. BAP-578 NFA contract (`0xd7Deb29ddBB13607375Ce50405A574AC2f7d978d`, env `BAP578_CONTRACT_ADDRESS`) IS functional on BSC (55K+ NFAs minted, 0.01 BNB mint fee) — registration requires sufficient BNB in user wallet. Functions: `ensureAgentRegisteredBSC()`, `isAgentRegistered()` in `token-launcher.ts`, `registerAgentERC8004()`, `registerAgentBAP578()` in `onchain.ts`. Telegram `/agentstatus` command checks registration status, with "Register Now" button for manual registration.
- **Module**: `server/token-launcher.ts` handles contract interactions and API signing for both platforms. Four.meme uses the `meme-api` v1 flow: nonce → wallet-signed login → API token create (simplified body: name, shortName, symbol, desc, imgUrl, label, raiseBnb → returns createArg + signature bytes) → on-chain `createToken(bytes,bytes)` on contract `0x5c952063c7fc8610FFDB798152D69F0B9550762b` with value = 0.01 BNB platform fee + raiseBnb. Pre-flight duplicate name check via four.meme search API. Flap.sh uses `newTokenV2(NewTokenV2Params)` on portal — requires vanity salt mining before TX submission, no API login needed.
