...
- **AI Trading**: Full-auto autonomous agent scans all 18 pairs every cycle using a multi-indicator strategy (EMA 8/21, RSI 14, MACD, Bollinger Bands, Volume analysis). Entry decisions are filtered through Aster DEX-specific intelligence: funding rate analysis (rejects trades paying excessive funding), orderbook imbalance detection (ensures pressure aligns with direction), and a self-learning confidence scorer that retrains weights every 20 trades. Users control max risk %, max leverage, max positions, TP/SL/trailing stop, funding filter, orderbook threshold, and AI confidence minimum. Agent decides pair, direction, leverage, and timing independently
- **Exit Management**: Take Profit (default 5%), Stop Loss (default 3%), Trailing Stop (default 2%), RSI overbought/oversold exits, signal reversal exits, and funding rate adverse exits
