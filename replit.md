...
- **AI Agent Badge (GMGN)**: ERC-8004 Identity Registry for AI badge on GMGN. BSC contract (`0x8004A818BFB912233c491871b3d84c89A494BD9e`) is a non-functional minimal proxy — BSC registration is skipped during token launches (auto-disabled after first failed call). Registration works on Base during agent creation. Functions: `ensureAgentRegisteredBSC()`, `isAgentRegistered()` in `token-launcher.ts`, `registerAgentERC8004()` in `onchain.ts`. Telegram `/agentstatus` command checks registration status, with "Register Now" button for manual registration.
