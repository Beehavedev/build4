...
- **Features**: Individual dashboard, deposit page, quick deposit buttons, manual and AI-assisted trading. AI agent uses Claude Sonnet as core brain for ALL trade decisions (not just a filter). Multi-timeframe analysis (5m/15m/1h) with key level detection. Iron-clad risk: max 1% per trade (code-enforced), hard stop-loss always, daily loss limit (default 3%), circuit breaker (3 consecutive losses → 30min pause), 5min cooldown after closes (anti-revenge), position sizing based on ATR volatility and stop distance. Claude provides full reasoning for every decision. Market regime detection (TRENDING/RANGING/VOLATILE via ADX+Choppiness Index) blocks RANGING trades. Live reasoning displayed in Mini App with decision log.
